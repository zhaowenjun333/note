...
class SnakeGame:
    ...
    def __init__(self):
        ...
        self.canvas.bind(54,lambda:self.turnto(0))
        self.canvas.bind(50,lambda:self.turnto(1))
        self.canvas.bind(52,lambda:self.turnto(2))
        self.canvas.bind(56,lambda:self.turnto(3))
...
appuifw.app.title=cn('《 贪 吃 蛇 》\n上:2下:8  左:4右:6')
playing=1
while playing:
    game=SnakeGame()
    game.run()
    playing=appuifw.query(cn('最后得分: %d \n\n再玩一次？')%game.score,'query')    
